...
def momentum_transfer_step(wa,mo_x,mo_y,te,alpha,gamma,decay=0.2,
    ...
    # Column contact coefficent
    cont_coef_d = np.minimum(1.0,cont_d/wa_div[:,:-1])
    cont_coef_a = np.minimum(1.0,cont_a/wa_div[:,1:])
    cont_coef_s = np.minimum(1.0,cont_s/wa_div[:-1,:])
    cont_coef_w = np.minimum(1.0,cont_w/wa_div[1:,:])
    # ### WATER ####
    # Discharge
    ...
    # Flow
    flow_d = cont_coef_d*(delta_t*np.maximum(0,mo_x[:,:-1])/h**3/density)
    flow_a = cont_coef_a*(delta_t*np.maximum(0,-mo_x[:,1:])/h**3/density)
    flow_s = cont_coef_s*(delta_t*np.maximum(0,mo_y[:-1,:])/h**3/density)
    flow_w = cont_coef_w*(delta_t*np.maximum(0,-mo_y[1:,:])/h**3/density)
    # Final transference
    ...
    # Generated momentums
    mo_gen_d = +gamma*disch_d*np.maximum(0,wa[:,:-1]-wa_f[:,1:])
    mo_gen_a = -gamma*disch_a*np.maximum(0,wa[:,1:]-wa_f[:,:-1])
    mo_gen_s = +gamma*disch_s*np.maximum(0,wa[:-1,:]-wa_f[1:,:])
    mo_gen_w = -gamma*disch_w*np.maximum(0,wa[1:,:]-wa_f[:-1,:])
    # Update momentums
    mo_x_f = mo_x.copy()
    mo_y_f = mo_y.copy()
    #
    mo_x_f[:,:-1] += mo_x_tran_a-mo_x_tran_d +mo_gen_a
    mo_x_f[:,1:] += mo_x_tran_d-mo_x_tran_a +mo_gen_d
