...
def ulti_2_step(w,u,v,d_t=0.01,d_x=1.0,gg=9.8,vv=1.0):
    # Discharges:
    ...
    tw_in_warp_inv = inv(warp(tw_in_4))
    # Total outgoing discharge
    tw_out = np.sum(disc,axis=0)
    tw_out_4 = np.array([tw_out,tw_out,tw_out,tw_out])
    tw_out_warp_inv = inv(warp(tw_out_4))
    # Outgoing mean height change
    k_out = (w_4-w_warp_inv)-0.5*tw_out_4+tw_out_warp_inv-0.5*tw_in_warp_inv
    # Momentum transference
    u_tran = disc*s_x_4
    v_tran = disc*s_y_4
    # Momentum generation:
    u_gen = np.zeros(w.shape)
    u_gen += disc[0]*((np.maximum(0,2*gg*k_out[0]+s_x**2))**0.5-s_x)
    u_gen += disc[1]*(-(np.maximum(0,2*gg*k_out[1]+s_x**2))**0.5-s_x)
    #
    v_gen = np.zeros(w.shape)
    v_gen += disc[2]*((np.maximum(0,2*gg*k_out[2]+s_y**2))**0.5-s_y)
    v_gen += disc[3]*(-(np.maximum(0,2*gg*k_out[3]+s_y**2))**0.5-s_y)
    # Water update:
