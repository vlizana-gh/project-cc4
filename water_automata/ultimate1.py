...
def ulti_1_step(w,u,v,d_t=0.01,d_x=1.0,gg=9.8,vv=1.0):
    # Discharges:
    ...
    u_gen += disc_warp[0]*((np.maximum(0,2*gg*k_inc[0]+s_x_warp[0]**2))**0.5-s_x_warp[0])
    u_gen += disc_warp[1]*(-(np.maximum(0,2*gg*k_inc[1]+s_x_warp[1]**2))**0.5-s_x_warp[1])
    #
    v_gen = np.zeros(w.shape)
    v_gen += disc_warp[2]*((np.maximum(0,2*gg*k_inc[2]+s_y_warp[2]**2))**0.5-s_y_warp[2])
    v_gen += disc_warp[3]*(-(np.maximum(0,2*gg*k_inc[3]+s_y_warp[3]**2))**0.5-s_y_warp[3])
    # Water update:
