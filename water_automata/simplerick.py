...
def simplerick_step(wa,mo_x,mo_y,delta_t=0.01,hh=1.0,gg=9.8,vel=10.0):
    ...
    disch[:,:,0] += np.maximum(0,mo_x)
    disch[:,:,1] += np.maximum(0,-mo_x)
    disch[:,:,2] += np.maximum(0,mo_y)
    disch[:,:,3] += np.maximum(0,-mo_y)
    disch *= delta_t/hh
    # New water:
    n_wa = wa.copy()
